...
struct mp4_stts_box mp4_read_stts_box(FILE *f, int size)  //level 8
{
	...
	fseek(f, cur_pos - sizeof(int) * box.number_of_entries , SEEK_SET);
...
bool stbl_need_to_offset(FILE *f, int curPos){
...
struct mp4_stbl_box mp4_read_stbl_box(FILE *f, int size)
{
    ...
    int box_size          = 0;
    //int cur_pos           = ftell(f) ;
	int cur_pos           = ftell(f); // GYJ
	if (stbl_need_to_offset(f, cur_pos)){
		cur_pos++;
	}
...
bool minf_need_to_offset(FILE *f, int curPos){
	fseek(f, curPos + 4, SEEK_SET);
	char szHeader[5] = { 0 };
	fread(szHeader, 1, 4, f);
	szHeader[4]= 0;
...
struct mp4_minf_box mp4_read_minf_box(FILE *f, int size)
{
    ...
    if (minf_need_to_offset(f, cur_pos)){
		cur_pos += 1;
	}
...
void mp4_read_trak_box(FILE *f, int size)
{
    printf("\t+%s\n", "trak");
...
/*final box
 */
void mp4_read_iods_box(FILE *f, int size)  //level 3
{
...
/*container box
  (mvhd, trak)
*/
void mp4_read_moov_box(FILE* f, int size)   //level 2
{
    printf("+%s\n",  "moov");
...
/*final box
 */
void mp4_read_ftyp_box(FILE *f, int size)  //level 2
{
    printf("+ftyp\n");
...
/*@a special final box
 */
void mp4_read_mdat_box(FILE *f, int size)
{
    printf("\t+%s\n", "mdat");
...
//	moov.trak[0];
	...
	for (int i = 0; i < sizeof(arrayIndex)/sizeof(int); i++)
	{
